...
import { ERR_UNKNOWN_RELAY_MANAGER, ERR_TXN_IS_EQUAL, ERROR_DIFFERENT_SIGNER, ERROR_DIFFERENT_NONCE, ERROR_TXNS_ALREADY_PENALIZED } from '../utils/errorMessages.utils';
...
describe('Penalizer Contract', function() {
    ...
    describe('Version Penalizer', function() {
        ...
        it('Should retrieve the penalizer version', async function() {
            const penalizerFactory: MockContractFactory<Penalizer__factory> = await smock.mock<Penalizer__factory>('Penalizer');
            
            penalizer = await penalizerFactory.deploy();
            expect(await penalizer.versionPenalizer(), 'Contract version is wrong').to.be.eql('2.0.1+enveloping.penalizer.ipenalizer');
        });
    })
    ...
    describe('Penalize Repeated nonce method() tests', function() {
        ...
        it('Should fail to penalize two equal txns', async function() {  
            ...
            const promiseOfPenalize = penalizer.connect(relayWorker).penalizeRepeatedNonce(
                txn.rawTxn,
                txn.signature,
                txn.rawTxn,
                txn.signature,
        ...
        it('Should failt to penalize a txns already penalized', async function() {
            ...
            await penalizer.connect(relayWorker).penalizeRepeatedNonce(
                unsignedTxn1.rawTxn,
                unsignedTxn1.signature,
                unsignedTxn2.rawTxn,
                unsignedTxn2.signature,
