import { TestTokenInstance, SmartWalletInstance, TestForwarderTargetInstance } from '../../types/truffle-contracts';
...
contract('SmartWallet contract - Unit testing on methods isInitialize and initialize', ([worker]) => {
    ...
    describe('Testing initialize and isInitialize methods and values for parameters', () => {
        ...
        it('Should verify method initialize fails with a null sender address parameter', async () => {
            ...
            await assert.isRejected(smartWallet.initialize(null,  token.address, worker, '0', '400000'));
        ...
        it('Should verify method initialize fails with a ZERO owner address parameter', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(constants.ZERO_ADDRESS, token.address, worker, '10', '400000'), 'Unable to pay for deployment', 'Error while validating data');
        ...
        it('Should verify method initialize fails with a null token address parameter', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(senderAddress, null, worker, '0', '400000'));
        ...
        it('Should verify method initialize reverts with negative gas amount', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(senderAddress, token.address, worker, '100', '-400000'));
        ...
        it('Should verify method initialize reverts with negative token amount', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(senderAddress, token.address, worker, '-2', '400000'));
        ...
        it('Should verify method initialize sucessfully with token as 0x address parameter', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isFulfilled(smartWallet.initialize(senderAddress, constants.ZERO_ADDRESS, worker, '10', '400000'));
        ...
        it('Should verify method initialize reverts with gas amount as null', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(senderAddress, token.address, worker, '0', null));
        ...
        it('Should verify method initialize sucessfully with all address type parameters as zero address', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isFulfilled(smartWallet.initialize(constants.ZERO_ADDRESS, constants.ZERO_ADDRESS, worker, '0', '400000'));
        ...
        it('Should verify method initialize successfully return with 0 tokens', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isFulfilled(smartWallet.initialize(senderAddress, token.address, worker, '0', '400000'));
        ...
        it('Should verify method initialize fails due to amount greater than 0 and gas less than 0', async () => {
            ...
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(senderAddress, token.address, worker, '10', '-10'));
        ...
        it('Should verify method initialize fails due to amount greater than 0 and ZERO token address', async () => {
            ...
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(senderAddress, token.address, worker, '0', '-400000'));
        ...
        it('Should verify method initialize fails when owner does not have funds to pay', async () => {
            ...
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(senderAddress, constants.ZERO_ADDRESS, worker, '10', '-400000'));
        ...
        it('Should verify method initialize fails when contract has already been initialized', async () => {
            ...
            await assert.isRejected(smartWallet.initialize(senderAddress, token.address, worker, '0', '400000'), 'already initialized', 'Error while validating data');
        });
...
contract('SmartWallet contract - Unit testing on method verify', ([worker]) => {
    ...
    describe('Testing verify method for values and parameters', () => {
        ...
        it('Should verify method verify revert because owner is not the owner of the smartwallet', async () => {
            ...
            await assert.isFulfilled(smartWallet.initialize(senderAccount.address, token.address, worker, '0', '400000'));
        ...
        it('Should verify method verify revert because of invalid domain separator', async () => {
            ...
            await assert.isFulfilled(
                smartWallet.initialize(senderAddress, token.address, worker, '0', '400000'));
...
contract('SmartWallet contract - Unit testing on execute method', ([worker]) => {
    describe('Testing execute method for values and parameters', () => {
        ...
        it('Should verify the method executed revert to Unable to pay relay', async () => {
            ...
            await smartWallet.initialize(senderAddress, token.address, worker, '0', '400000');
...
contract('SmartWallet contract - Unit testing on directExecute method', ([worker, fundedAccount]) => {
    describe('Testing directExecute method for values and parameters', () => {
        ...
        it('Should call succesfully to method directExecute through node funded account', async () =>{
            ...
            await smartWallet.initialize(fundedAccount, token.address, worker, '0', '400000');

            assert.isTrue(await smartWallet.isInitialized());
            const initialNonce = await smartWallet.nonce();

            await assert.isFulfilled(smartWallet.directExecute(recipient.address, recipientFunction, {from: fundedAccount}));
