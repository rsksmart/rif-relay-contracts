...
import {
    EIP712TypedData,
    signTypedData_v4, TypedDataUtils
} from 'eth-sig-util';
import { BN, bufferToHex, toBuffer, privateToAddress } from 'ethereumjs-util';
import TypedRequestData, { ForwardRequestType } from '../../types/EIP712/TypedRequestData';
...
contract('Custom SmartWallet contract - Unit testing on method verify', ([worker]) => {
    ...
    describe('Testing verify method for values and parameters', () => {
        ...
        it('Should verify method verify revert because owner is not the owner of the smartwallet', async () => {
            ...
            await assert.isRejected(smartWallet.verify(suffixData, relayRequest.request, signature), 'Not the owner of the SmartWallet', 'Error while validating the owner');
        });
        ...
        it('Should verify method verify and revert because of signature mismatch', async () => {
            ...
            await assert.isRejected(smartWallet.verify(suffixData, relayRequest.request, signature), 'Signature mismatch', 'Error while validating data');
        });
        ...
        it('Should verify method successfully sign a txn', async () => {
            ...
            const { signature, suffixData } = signRequest(senderPrivateKey, relayRequest, chainId);
            await assert.isFulfilled(smartWallet.verify(suffixData, relayRequest.request, signature));
        });
    });
});
...
contract('Custom SmartWallet contract - Unit testing on execute method', ([worker]) => {
    describe('Testing execute method for values and parameters', () => {
        ...
        it('Should verify the method executed revert to the Invalid caller', async () => {
            ...
            await assert.isRejected(smartWallet.execute(suffixData, relayRequest.request, signature, { from: worker }), 'Invalid caller', 'Error while validating the data');
        });
        ...
        it('Should verify the method executed revert to Unable to pay relay', async () => {
           ...
           const relayRequest = createRequest({
            ...
            await assert.isRejected(smartWallet.execute(suffixData, relayRequest.request, signature, { from: worker }), 'Unable to pay for relay', 'Error while validating the data');
        });
        ...
        it('Should verify the method executed reverts to Not enough gas', async () => {
            ...
            await assert.isRejected(smartWallet.execute(suffixData, relayRequest.request, signature, { from: worker }), 'Not enough gas left', 'Error while validating the data');
        });
        ...
        it('Should verify the method executed sucess for transfer with 0 tokenAmount', async () => {
            ...
            await assert.isFulfilled(smartWallet.execute(suffixData, relayRequest.request, signature, { from: worker }));
