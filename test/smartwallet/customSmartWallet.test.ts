import { TestTokenInstance, CustomSmartWalletInstance, SuccessCustomLogicInstance, TestForwarderTargetInstance } from '../../types/truffle-contracts';
import chai from 'chai';
import chaiAsPromised from 'chai-as-promised';
import { EIP712TypedData, // @ts-ignore 
signTypedData_v4, TypedDataUtils } from 'eth-sig-util';
import { BN, bufferToHex, toBuffer, privateToAddress } from 'ethereumjs-util';
import { TypedRequestData, getDomainSeparatorHash, ForwardRequestType, constants, RelayRequest, ForwardRequest, RelayData } from '@rsksmart/rif-relay-common';
import { bytes32, getTestingEnvironment, containsEvent} from '../utils/TestUtils';
...
async function getTokenBalance (token: TestTokenInstance, account: string): Promise<BN> {
   ...
   * @param amount 
   */
  async function mintTokens (token: TestTokenInstance, recipient: string, amount: string): Promise<void> {
...
contract('Custom SmartWallet contract - Unit testing on methods isInitialize and initialize', ([worker]) => {
    ...
    describe('Testing initialize and isInitialize methods and values for parameters', () => {
        ...
        it('Should verify method initialize fails with a null sender address parameter', async () => {
            ...
            await assert.isRejected(smartWallet.initialize(null, customLogic.address, token.address, worker, '0', '400000', '0x'));
        ...
        it('Should verify method initialize fails with a ZERO owner address parameter', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(constants.ZERO_ADDRESS, customLogic.address, token.address, worker, '10', '400000', '0x'), 'Unable to pay for deployment', 'Error while validating data');
        ...
        it('Should verify method initialize fails with a null token address parameter', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(senderAddress, customLogic.address, null, worker, '0', '400000', '0x'));
        ...
        it('Should verify method initialize reverts with negative gas amount', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(senderAddress, customLogic.address, token.address, worker, '100', '-400000', '0x'));
        ...
        it('Should verify method initialize reverts with negative token amount', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(senderAddress, customLogic.address, token.address, worker, '-2', '400000', '0x'));
        ...
        it('Should verify method initialize sucessfully with token as 0x address parameter', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isFulfilled(smartWallet.initialize(senderAddress, customLogic.address, constants.ZERO_ADDRESS, worker, '10', '400000', '0x'));
        ...
        it('Should verify method initialize reverts with gas amount as null', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(senderAddress, customLogic.address, token.address, worker, '0', null, '0x'));
        ...
        it('Should verify method initialize sucessfully with all address type parameters as zero address', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isFulfilled(smartWallet.initialize(constants.ZERO_ADDRESS, constants.ZERO_ADDRESS, constants.ZERO_ADDRESS, worker, '0', '400000', '0x'));
        ...
        it('Should verify method initialize successfully return with 0 tokens', async () => {
            ...
            assert.isFalse(await smartWallet.isInitialized());
            //Initializaing the contract
            await assert.isFulfilled(smartWallet.initialize(senderAddress, customLogic.address, token.address, worker, '0', '400000', '0x'));
        ...
        it('Should verify method initialize fails due to amount greater than 0 and gas less than 0', async () => {
            ...
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(senderAddress, customLogic.address, token.address, worker, '10', '-10', '0x'));
        ...
        it('Should verify method initialize fails due to amount greater than 0 and ZERO token address', async () => {
            ...
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(senderAddress, customLogic.address, token.address, worker, '0', '-400000', '0x'));
        ...
        it('Should verify method initialize fails when owner does not have funds to pay', async () => {
            ...
            //Initializaing the contract
            await assert.isRejected(smartWallet.initialize(senderAddress, customLogic.address, constants.ZERO_ADDRESS, worker, '10', '-400000', '0x'));
        ...
        it('Should verify method initialize fails when contract has already been initialized', async () => {
            ...
            await assert.isRejected(smartWallet.initialize(senderAddress, customLogic.address, token.address, worker, '0', '400000', '0x'), 'already initialized', 'Error while validating data');
        });
        //TODO might need to include scenarios where the logic address is not 0x
    });
});

contract('Custom SmartWallet contract - Unit testing on method verify', ([worker]) => {
    ...
    describe('Testing verify method for values and parameters', () => {
        ...
        it('Should verify method verify revert because owner is not the owner of the smartwallet', async () => {
            ...
            await assert.isFulfilled(smartWallet.initialize(senderAccount.address, customLogic.address, token.address, worker, '0', '400000', '0x'));

            assert.isTrue(await smartWallet.isInitialized());

            chainId = (await getTestingEnvironment()).chainId;
            const domainSeparatorHash = getDomainSeparatorHash(smartWallet.address, chainId);
            ...
            await assert.isRejected(smartWallet.verify(domainSeparatorHash, suffixData, relayRequest.request, signature), 'Not the owner of the SmartWallet', 'Error while validating the owner');
        });
        ...
        it('Should verify method verify revert because nonce mismatch', async () => {
            ...
            const relayRequest = createRequest(
                ...
                await assert.isRejected(smartWallet.verify(
                    ...
                    signature), 'nonce mismatch', 'Error while validating data');
        });

        it('Should verify method verify revert because of invalid domain separator', async () => {
            assert.isFalse(await smartWallet.isInitialized());
            ...
            const senderPrivateKey = toBuffer(bytes32(1));
                ...
                await assert.isRejected(smartWallet.verify(
                    ...
                    suffixData,
                    relayRequest.request,
                    signature), 'Invalid domain separator', 'Error while validating data');
        });
        ...
        it('Should verify method verify and revert because of signature mismatch', async ()=>{
            ...
            const relayRequest = await createRequest(
                ...
                await assert.isRejected(smartWallet.verify(domainSeparatorHash, suffixData, relayRequest.request, signature), 'signature mismatch', 'Error while validating data');
        });  
        ...
        it('Should verify method successfully sign a txn', async () =>{
            ...
            const relayRequest = createRequest(
                ...
                const { signature, suffixData } = signRequest(senderPrivateKey, relayRequest, chainId);
                await assert.isFulfilled(smartWallet.verify(domainSeparatorHash, suffixData, relayRequest.request, signature));
        }); 
    });
});

contract('Custom SmartWallet contract - Unit testing on execute method', ([worker]) => {
    describe('Testing execute method for values and parameters', () => {
        ...
        it('Should verify the method executed revert to the Invalid caller', async () => {
            ...
            await assert.isRejected(smartWallet.execute(domainSeparatorHash, suffixData, relayRequest.request, signature, { from: worker }), 'Invalid caller', 'Error while validating the data');
        });
        ...
        it('Should verify the method executed revert to Unable to pay relay', async () => {
            ...
            await assert.isRejected(smartWallet.execute(domainSeparatorHash, suffixData, relayRequest.request, signature, { from: worker }),'Unable to pay for relay', 'Error while validating the data');
        });
        ...
        it('Should verify the method executed reverts to Not enough gas', async () => {
            ...
            await assert.isRejected(smartWallet.execute(domainSeparatorHash, suffixData, relayRequest.request, signature, { from: worker }),'Not enough gas left', 'Error while validating the data');
        });

        it('Should verify the method executed sucess for transfer with 0 tokenAmount', async () => {
            ...
            await assert.isFulfilled(smartWallet.execute(domainSeparatorHash, suffixData, relayRequest.request, signature, { from: worker }));
            assert.equal((await smartWallet.nonce()).toString(), initialNonce.add(new BN(1)).toString(), 'Calling execute method should increment nonce');
        });
        ...
        it('Should verify the method executed success to same sender and receiver', async () => {
            ...
            relayRequest.request.gas = '100';
            const { signature, suffixData } = signRequest(senderPrivateKey, relayRequest, chainId);
            await mintTokens(token, smartWallet.address, transferAmount.toString());

            await assert.isFulfilled(smartWallet.execute(domainSeparatorHash, suffixData, relayRequest.request, signature, { from: worker }));
            assert.equal((await smartWallet.nonce()).toString(), initialNonce.add(new BN(1)).toString(), 'Calling execute method should increment nonce');
            assert.equal(initialBalance.toString(), (getTokenBalance(token, senderAddress)).toString(), 'Sender balance do not match');
        });
        ...
        it('Should verify balances after a transaction between accounts', async () => {
            ...
            assert.equal((await getTokenBalance(token, smartWallet.address)).toString(), (new BN(transferAmount)).toString(), 'Smart wallet token balance was not increased');
            ...
            assert.equal(tknBalance.toString(), initialWorkerTokenBalance.add(new BN(transferAmount)).toString(), 'Worker token balance did not change');
            assert.equal(swTknBalance.toString(), initialSWalletTokenBalance.toString(), 'Smart wallet token balance did not change');
            assert.equal((await smartWallet.nonce()).toString(), initialNonce.add(new BN(1)).toString(), 'Call to execute should increment nonce');
        });
    });
});

contract('Custom SmartWallet contract - Unit testing on directExecute method', ([worker, fundedAccount]) => {
    describe('Testing directExecute method for values and parameters', () => {
        ...
        it('Should revert call to directExecute with empty parameter', async () => {
            await assert.isRejected(smartWallet.directExecute('', recipientFunction, {from: fundedAccount}));
        });

        it('Should revert call to directExecute with null parameter', async () => {
            await assert.isRejected(smartWallet.directExecute(recipient.address, null, {from: fundedAccount}));
        });

        it('Should revert call to directExecute with no funded account as parameter', async () => {
            await assert.isRejected(smartWallet.directExecute(recipient.address, recipientFunction),'Not the owner of the SmartWallet', 'Error while validating the data');
        });

        it('Should revert call to directExecute with INVALID_ARGUMENT as parameter', async () =>{
           await  assert.isRejected(smartWallet.directExecute('', recipientFunction, {from: fundedAccount}),'INVALID_ARGUMENT', 'Error while validating the data');
        });
        ...
        it('Should call succesfully to method directExecute through node funded account', async () =>{
            ...
            assert.equal((await smartWallet.nonce()).toString(), initialNonce.toString(), 'Call to direct execute should NOT increment nonce');
            // @ts-ignore
