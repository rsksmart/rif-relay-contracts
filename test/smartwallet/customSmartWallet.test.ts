...
describe('CustomSmartWallet contract', function () {
  ...
  describe('Function initialize()', function () {
    ...
    async function createCustomSmartWalletFactory(owner: Wallet) {
      const customSmartWalletTemplateFactory = await hardhat.getContractFactory(
        'CustomSmartWallet'
      );
      const customSmartWalletTemplate =
        await customSmartWalletTemplateFactory.deploy();

      const customSmartWalletFactoryFactory = await hardhat.getContractFactory(
        'CustomSmartWalletFactory'
      );
      customSmartWalletFactory = await customSmartWalletFactoryFactory
        .connect(owner)
        .deploy(customSmartWalletTemplate.address);
    }
    ...
    it('Should initialize a CustomSmartWallet', async function () {
      ...
      const customSmartWalletAddress =
      ...
      const customSmartWallet = await hardhat.getContractAt(
        'CustomSmartWallet',
    ...
    it('Should fail to initialize an already initialized CustomSmartWallet', async function () {
      const privateKey = Buffer.from(owner.privateKey.substring(2, 66), 'hex');
  ...
  describe('Function verify()', function () {
    ...
    beforeEach(async function () {
      ...
      const mockCustomSmartWalletFactory =
      ...
      mockCustomSmartWallet = await mockCustomSmartWalletFactory
        .connect(owner)
        .deploy();
      ...
      await mockCustomSmartWallet.setVariable(
        'domainSeparator',
        domainSeparator
      );
    });
  ...
  describe('Function execute()', function () {
    ...
    beforeEach(async function () {
      let fundedAccount: SignerWithAddress;
      [fundedAccount, relayHub] = (await hardhat.getSigners()).slice(1) as [
        SignerWithAddress,
        SignerWithAddress
      ];
  ...
  describe('Function directExecute()', function () {
    ...
    beforeEach(async function () {
      [fundedAccount] = (await hardhat.getSigners()).slice(1) as [
        SignerWithAddress
      ];
  ...
  describe('Function getOwner()', function () {
    ...
    it('Should return the encrypted owner', async function () {
      const returnedOwner = await mockCustomSmartWallet.getOwner();
