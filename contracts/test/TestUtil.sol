...
contract TestUtil {
    bytes32 public constant RELAY_REQUEST_TYPEHASH =
        keccak256(
            'RelayRequest(address relayHub,address from,address to,address tokenContract,address collectorContract,uint256 value,uint256 gas,uint256 nonce,uint256 tokenAmount,uint256 tokenGas,bytes data,RelayData relayData)RelayData(uint256 gasPrice,address relayWorker,address callForwarder,address callVerifier)'
        );
    ...
    function callForwarderVerifyAndCall(
        ...
        (forwarderSuccess, success, ret) = Eip712Library.execute(
            relayRequest,
            signature
        );
    ...
    function splitRequest(EnvelopingTypes.RelayRequest calldata req)
        external
        pure
        returns (
