...
contract TestVerifierVariableGasLimits is TestVerifierEverythingAccepted {
    string public override versionVerifier =
        '2.0.1+enveloping.test-vgl.iverifier';

    event SampleRecipientPreCallWithValues(uint256 gasleft);

    event SampleRecipientPostCallWithValues(uint256 gasleft);
