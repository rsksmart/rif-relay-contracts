...
contract TestRecipient {
    ...
    event SampleRecipientEmittedNew(
        string message,
        address msgSender,
        address origin,
        uint256 msgValue,
    ...
    function emitMessage3(string memory message)
        public
        payable
        returns (string memory)
    {
        ...
        emit SampleRecipientEmittedNew(
            'result',
            msg.sender,
            tx.origin,
            msg.value,
    ...
    function checkReturnValues(uint256 len, bool doRevert)
        public
        view
        returns (string memory)
    {
        (this);
        string
            memory mesg = "this is a long message that we are going to return a small part from. we don't use a loop since we want a fixed gas usage of the method itself.";
